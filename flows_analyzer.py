...
def main(pcap_file, live, list_interfaces, interface, port, duration, chart, overall, generate_data_charts, 
    ...
    try:
        if overall:
            ...
            large_flows = detect_large_flow(pcap_file, ip_to_remove, chart)
            
            print("Detecting asymmetrical flows...")
            asymmetrical_flows = asymmetrical_flow(pcap_file, ip_to_remove, chart)
            
            print("Detecting unusual port usage...")
            unusual_ports = unusual_ports_flow(pcap_file, ip_to_remove, chart)
            
            print("Finding DNS users...")
            dns_users = find_DNS_users(pcap_file, ip_to_remove, chart)
            
            print("Detecting SYN flood attacks...")
            syn_flood = detect_SYN_flood(pcap_file, ip_to_remove, chart)
            
            print("Detecting HTTP GET requests...")
            http_get = detect_http_get(pcap_file, ip_to_remove, chart)
            
            print("Detecting Ping flood attacks...")
            ping_flood = detect_ping_flood(pcap_file, ip_to_remove, chart)
            
            print("Detecting port scanning...")
            port_scanning = detect_ports_scanner(pcap_file, ip_to_remove, chart)
            
            print("Sigma detection...")
            sigma_detections = run_sigma_analysis(pcap_file)
            sigma_detections = print_detections(sigma_detections)
            ...
            results_dict.update({
                ...
                'asymmetrical_flows': asymmetrical_flows,
            ...
            generate_ip_location_map(large_flows, 
                                asymmetrical_flows,
                                unusual_ports,
                                dns_users,
                                syn_flood,
